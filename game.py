...
class TicTacToe:
    ...
    def winner(self, square, letter):
        # winner if 3 in row or anywhere... we have to check
        ...
        # these are the only moves possible to win diagonals
        if square % 2 == 0:
...
def play(game, x_player, o_player):
    ...
    while game.empty_squares():
        ...
        if letter == "X":
            print(f"You make a move to spot {square}")
        else:
            print(f"Computer make a move to spot {square}")
        game.print_board()
        print()
